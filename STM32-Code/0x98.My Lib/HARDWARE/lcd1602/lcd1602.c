...
void Read_Busy() {
	...
	GPIO_InitSturcture.GPIO_Mode  = GPIO_Mode_IN_FLOATING;//读取数据前转为浮空输入
	GPIO_InitSturcture.GPIO_Pin   = GPIO_Pin_15;
	GPIO_Init(GPIOB,&GPIO_InitSturcture);
	...
	GPIO_InitSturcture.GPIO_Mode  = GPIO_Mode_Out_PP;//换回推挽输出
	GPIO_InitSturcture.GPIO_Pin   = GPIO_Pin_15;
	GPIO_InitSturcture.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB,&GPIO_InitSturcture);
