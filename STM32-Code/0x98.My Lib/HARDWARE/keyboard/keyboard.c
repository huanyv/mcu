...
uint8_t Read_Key(void) {
	...
	GPIO_Write(GPIOA,GPIO_ReadOutputData(GPIOA) & 0Xff00);//不影响其它位操作
	GPIO_Write(GPIOA,GPIO_ReadOutputData(GPIOA) | 0X0F);//PA0`3至为高电平，PA4`7至为低电平
	...
	if((key_song == 0)&&((GPIO_ReadInputData(GPIOA)&0x00ff) != 0x000f)) {
		...
		GPIO_Write(GPIOA,GPIO_ReadOutputData(GPIOA) | 0xf0);//低八位电平翻转
