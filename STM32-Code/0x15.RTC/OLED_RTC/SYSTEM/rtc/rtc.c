...
void RTC_Init(void) {
	...
	if(BKP_ReadBackupRegister(BKP_DR1) != 0xAAAA) {   // 如果后备寄存器非 预写值，初始化RTC 
		BKP_DeInit(); // 复位后备寄存器
		...
		RTC_Set(2019,6,13,11,26,0); // 初始化时间，
		
		BKP_WriteBackupRegister(BKP_DR1,0xAAAA); // 写后备寄存器
	}
...
void RTC_IRQHandler(void) { // RTC中断服务函数
	if(RTC_GetITStatus(RTC_IT_SEC) != RESET) { // 如果检测到秒中断，也就是1s后
		RTC_Get();   // 更新时间
		RTC_WaitForLastTask();
