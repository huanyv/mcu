/***********************************************************************			   ||||||||||||||||||||||
初始界面->按下k4进入设置->时间暂停->k3调整实时时间->不显示光标后时间启动			   |闹钟：青花瓷（可改）|
																					   |闹钟时间断电自动存储|
...
uchar code zi[3][8]={{0x10,0x06,0x09,0x08,0x08,0x09,0x06,0x00},	//摄氏度图标
				{0x0A,0x1F,0x15,0x17,0x11,0x0E,0x1B,0x00},	//闹钟图标
				{0x00,0x04,0x0E,0x1F,0x1F,0x1F,0x00,0x00}};	//选中图标

uchar code week_lcd[][6]={{"error"},{"Mon."},{"Tues."},{"Wed."},{"Thur."},{"Fri."},{"Sat."},{"Sun."}};	//星期显示二维数组
...
void buling()	  //闹钟触发
{
	...
	{
		t1=ds1302_read(0x83);t2=ds1302_read(0x85);	  //读出 分钟和小时
	  	temp_min =((t1&0xf0)>>4)*10+(t1&0x0f);	//BCD码转
		temp_hour=((t2&0xf0)>>4)*10+(t2&0x0f);			//十进制码
		if(min==temp_min&&ii==0)		//分钟对比		
		{
			if(hour==temp_hour&&ii==0)		//如果分钟相同   小时对比
			{
				if(flag==0&&ii==0)		//只有在初始界面才会响
				{
					lcd1602_write_0(0x01);	
					delay_lcd(2500);
					lcd1602_write_0(0x48);
					...
					TMOD = 1; ET0 = 1;EA = 1; 	 	//定时器  初始化
					i2 = 0;
					time02 = 1; 
					while(ii==0)	//ii=1 的 控制 在定时器中断里面	  ii=1时退出循环 停止响铃 
					{
						k1 = sszymmh[i2] + 7 * sszymmh[i2 + 1] - 1;
						//第i2个是音符, 第i2+1个是第几个八度
						timer0h = FREQH[k1]; //从数据表中读出频率数值
						timer0l = FREQL[k1]; //实际上, 是定时的时间长度
						time02 = sszymmh[i2 + 2]; //读出时间长度数值
						i2 += 3;
						song(); //发出一个音符
					}
					ii=0;
...
void read_at24c02()//从AT24C02里读函数
{
	hour=read_addr(0x00);
	if(hour>23)hour=0;
	min=read_addr(0x01);
...
void int1() interrupt 2//外部 中断 1	 光标控制
{
	delay_lcd(2000);
	if(k4==0)
	{
...
void int0() interrupt 0//外部 中断 0	   时间更改
{
	uchar temp_1=0;
	delay_lcd(2000);
	if(k3==0)
