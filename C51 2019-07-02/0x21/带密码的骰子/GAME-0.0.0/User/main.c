	...
	if(Key_Count != 0) {//如果有按键按下
		...
		if(MEUN1==4) {
			...
			if(mima[0] == 0&&mima[1] == 1&&mima[2] == 2&&mima[3] == 3) {//密码正确
				...
				for(i=0;i<4;i++) {
					DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();
					DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();
					DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();
					DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();
					DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();
					DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();DisPlay_Init();//显示 错误 跳动  有待完善
					delay(50000);
				}
				for(i=0;i<4;i++) {
					mima[i]=16;//回归 四个   '-'
