...
uchar temperature[]="Temperature:";			   //LCD标题
...
{
    uint i,j,k=2;
    uchar table1[]= {0x10,0x06,0x09,0x08,0x08,0x09,0x06,0x00};  			//自定义   度 C  符号
    time0init();
    LCD_init();
    zidingyi(table1,8);
    LCD_X_Y(2,11,0x00);									//显示 第一个字符到  2行11列
    for(i=0; temperature[i]!='\0'; i++)
    {
        LCD_X_Y(1,k++,temperature[i]);
        delay(100);
    }
